...
def main(args):
    ...
    # save config
    yaml_dir = os.path.join(ckpt_dir, 'config.yaml')
    ...
    with open(stats_path, 'r') as f:
        np.savez(stats_path.replace('.pkl', '.npz'), **stats)
...
def make_policy(policy_class, config):
    if policy_class == 'ACT':
...
def forward_pass(data, policy, device='cpu'):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_dir', action='store', type=str, help='dataset_dir', default=None)
    parser.add_argument('--episode_len', action='store', type=int, default=None)
